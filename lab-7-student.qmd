...
fish |>
  ...
  filter(number_missing > 0) |>
   mutate(obs_missing = fish |> 
           filter(if_any(everything(), 
                         ~ is.na(.))) |> 
           summarize(obs_missing = n()) |> 
           pull(obs_missing))
...
fish |>
  ...
  geom_line() +
  geom_point() +
  labs(title = "Count of Missing Values by Year",
       x = "Year",
       y = "") +
  theme_minimal()
```
...
The wrong variable was used in the weight calculation.
...
fish |>
  ...
  labs(x = "Re-Scaled Values of Fish Length (mm)") +
  scale_y_continuous(limits = c(0,
                                4000))
```
